...
<v-mini-cart>
    @if ($type === "desktop")
        <a class="relative cr-right-bar-item Shopping-toggle transition-all duration-[0.3s] ease-in-out flex items-center" role="button"
            aria-label="@lang('licious::app.checkout.cart.mini-cart.shopping-cart')"
            tabindex="0">
    ...
    >
        ...
        <x-licious::drawer isActive="false" :append="'cr-cart-view right-[0]'">
            ...
            <x-slot:toggle>
                ...
                @if ($type === "desktop")
                    <a href="javascript:void(0)" @click.prevent="open" class="relative cr-right-bar-item Shopping-toggle transition-all duration-[0.3s] ease-in-out flex items-center" role="button"
                        aria-label="@lang('licious::app.checkout.cart.mini-cart.shopping-cart')"
                        tabindex="0">
                        <span>
                            <i class="ri-shopping-cart-line pr-[5px] text-[21px] leading-[17px]"></i>
             ...
             <div class="cr-cart-inner w-[350px] h-full m-[0] px-[20px] bg-[#fff] relative z-[9] flex flex-col justify-between max-[575px]:w-[300px] max-[360px]:w-[280px]">
                <div class="cr-cart-top text-[#000]">
                    ...
                    <div class="cr-cart-title mb-[15px] py-[15px] flex flex-row justify-between items-center border-b-[1px] border-solid border-[#e9e9e9]">
                        <h6 class="m-[0] text-[17px] font-bold text-[#2b2b2d] leading-[1.2]">@lang('licious::app.checkout.cart.mini-cart.shopping-cart')</h6>
                    ...
                    >
                        <li
                            class="mb-[20px] pb-[20px] flex border-b-[1px] border-solid border-[#e9e9e9]"
                        ...
                        :key="item?.id">
                            ...
                            <div class="cr-pro-content pl-[15px] relative grow-[1] basis-[70%]">
                                ...
                                <div class="cart-price mt-[5px] text-[14px] flex gap-x-2 items-center">
                                    <span v-text="item.formatted_price" class="text-[#777] font-bold text-[16px]"></span>
                                    {!! view_render_event('bagisto.shop.checkout.mini-cart.drawer.content.price.after') !!}

                                    <!-- Cart Item Options Container -->
                                    <div
                                        class="grid gap-x-2.5 gap-y-1.5 select-none relative z-40"
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <span
                                                class="text-2xl text-[#777]"
                                                :class="{'ri-arrow-up-s-line': item.option_show, 'ri-arrow-down-s-line': ! item.option_show}"
                                            ></span>
                                        </button>

                                        <!-- Option Details -->
                                        <div class="grid gap-2 min-w-36 p-2 bg-white rounded-lg shadow-md" :class="{ 'hidden': !item.option_show, 'absolute z-1050 top-6': item.option_show }">
                                            <div class="py-1" v-for="option in item.options">
                                ...
                                <div class="cr-cart-qty mt-[5px]">
                                    ...
                                    <!-- Cart Item Quantity Changer -->
                                     <div  class="cart-qty-plus-minus m-[0] w-[80px] h-[30px] relative overflow-hidden flex bg-[#fff] border-[1px] border-solid border-[#e9e9e9] rounded-[5px] items-center justify-between">
                                        <button type="button" class="plus h-[25px] w-[25px] mt-[-2px] border-[0] bg-transparent flex justify-center items-center" @click="updateItem('increase', item)">+</button>
                                        <input type="text" name="quantity" placeholder="." :value="item?.quantity" minlength="1" maxlength="20" class="quantity w-[30px] m-[0] p-[0] text-[#444] float-left text-[14px] font-semibold leading-[38px] h-auto text-center outline-[0]">
                                        <button type="button" class="minus h-[25px] w-[25px] mt-[-2px] border-[0] bg-transparent flex justify-center items-center" @click="updateItem('decrease', item)">-</button>
                                    </div>
                                ...
                                <a href="javascript:void(0)" @click="removeItem(item.id)" class="remove py-[0] px-[9px] absolute top-[0] right-[0] text-[17px] leading-[15px] bg-[#fff] text-[#fb5555]">×
                                    <span class="sr-only">@lang('licious::app.checkout.cart.mini-cart.remove')</span>
                                </a>
                ...
                <div v-if="cart?.items?.length" class="cr-cart-bottom relative top-[-20px]">
                    ...
                    {!! view_render_event('bagisto.shop.checkout.mini-cart.subtotal.before') !!}
                        <table v-if="! isLoading" class="table cart-table mt-[10px] w-full">
                            <tbody>
                                <tr>
                                    <td class="text-left text-[16px] text-[#000] font-normal py-[7px]">@lang('licious::app.checkout.cart.mini-cart.subtotal') :</td>
        ...
        app.component("v-mini-cart", {
            ...
            methods: {
                ...
                updateItem(change, item) {
                    const quantity = this.getQty(change, item.quantity);
