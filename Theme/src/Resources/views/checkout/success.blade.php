<x-licious::layouts>
    <!-- Page Title -->
    <x-slot:title>
		@lang('licious::app.checkout.success.thanks')
    </x-slot>
		...
		<div class="grid gap-y-5 place-items-center">
			...
			<p class="text-xl">
				@if (auth()->guard('customer')->user())
					@lang('licious::app.checkout.success.order-id-info', [
						'order_id' => '<a class="text-[#0A49A7]" href="' . route('shop.customers.account.orders.view', $order->id) . '">' . $order->increment_id . '</a>'
					])
				@else
					@lang('licious::app.checkout.success.order-id-info', ['order_id' => $order->increment_id])
				@endif
			</p>
			...
			<p class="text-xl text-[#6E6E6E]">
				...
				@else
					@lang('licious::app.checkout.success.info')
				@endif
			</p>
